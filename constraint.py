...
class Constraint2D:
    ...
    def intersection_with(self, other):
        assert(isinstance(other, Constraint2D))
        # Subtract the two equations: find the x coefficient and constant

        # edge case where the function is a vertical line, of the form x = k
        if self.y_coefficient == 0:
            return (self.constant, other.as_line(self.constant))
        elif other.y_coefficient == 0:
            return (other.constant, self.as_line(other.constant))
        try:
